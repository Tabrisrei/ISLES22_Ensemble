...
class ThresholdModel():
    def __init__(self,
        ...
        if self.debug:
            self._input_path = Path('/home/edelarosa/Documents/ensemble/nvauto/isles22/test/')
            self._output_path = Path('/home/edelarosa/Documents/ensemble/nvauto/isles22/test/output')
    ...
    def predict(self, input_data):
        ...
        model_inferer = SlidingWindowInferer(roi_size=[192, 192, 128], overlap=0.625, mode='gaussian',
                                             cache_roi_weight_map=True, sw_batch_size=2,
                                             device='cpu', sw_device='cpu')
            ...
            for idx, batch_data in enumerate(val_loader):
                ...
                for checkpoint in checkpoints:
                    ...
                    with autocast(enabled=True):
                        logits = model_inferer(inputs=image.to('cpu'),
                                               network=model.to('cpu'))  # another inferer (e.g. sliding window)

                    probs = torch.softmax(logits.float(), dim=1)

                    batch_data["pred"] = probs
                    inverter = transforms.Invertd(keys="pred", transform=val_transform, orig_keys="image",
                                                  meta_keys="pred_meta_dict", nearest_interp=False, to_tensor=True)
    ...
    def load_isles_case(self):
        ...
        input_data = {'dwi_image': SimpleITK.ReadImage(str(dwi_image_path)), 'dwi_image_path': str(dwi_image_path),
                      ...
                      'flair_image_path': str(flair_image_path), 'flair_json': json.load(open(flair_json_path))}
