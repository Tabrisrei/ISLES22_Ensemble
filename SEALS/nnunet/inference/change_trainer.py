...
def pretend_to_be_other_trainer(folder, new_trainer_name, checkpoints=("model_best.model.pkl", "model_final_checkpoint.model.pkl")):
    ...
    for c in checkpoints:
        for f_index, f in enumerate(folds):
            checkpoint_file = join(folder, f, c)
            if isfile(checkpoint_file):
                a = load_pickle(checkpoint_file)
                a['name'] = new_trainer_name
                a['init'] = ('data/nnUNet_preprocessed/Task011_Ischemic_Stroke_Two_Modality/nnUNetPlansv2.1_plans_3D.pkl', f_index, 'data/nnUNet_trained_models/nnUNet/3d_fullres/Task011_Ischemic_Stroke_Two_Modality/nnUNetTrainerV2_DDP__nnUNetPlansv2.1', 'data/nnUNet_preprocessed/Task011_Ischemic_Stroke_Two_Modality', False, 0, True, False, True)
